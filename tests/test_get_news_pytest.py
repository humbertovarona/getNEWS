...
class TestGetNews:
    ...
    def test_skips_articles_with_removed_title(self):
        ...
        conn = setup_database()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO news (date_time, title, source, category, url) VALUES (?, ?, ?, ?, ?)",
                       ('2021-01-01', '[Removed]', 'Source', 'Category', 'https://example.com'))
        conn.commit()
    ...
    def test_skips_articles_already_saved_in_database(self):
        ...
        cursor = conn.cursor()
        cursor.execute("INSERT INTO news (date_time, title, source, category, url) VALUES (?, ?, ?, ?, ?)",
                       ('2021-01-01', 'Article Title', 'Source', 'Category', 'https://example.com'))
        conn.commit()
        ...
        # Assert
        cursor.execute("SELECT COUNT(*) FROM news WHERE title = 'Article Title'")
        result = cursor.fetchone()
        assert result[0] == 1
        conn.close()
